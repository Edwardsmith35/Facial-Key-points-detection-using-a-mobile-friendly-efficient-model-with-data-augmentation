...
def save_checkpoint(epoch, model, optimizer, train_loss_history, val_loss_history, save_dir, filename_prefix=CHECKPOINT_FILENAME_PREFIX):
...
def load_checkpoint(model, optimizer, save_dir, filename_prefix=CHECKPOINT_FILENAME_PREFIX, learning_rate=None):
    """
    ...
    if latest_checkpoint_file:
        ...
        try:
            ...
            current_lr = optimizer.param_groups[0]['lr'] if optimizer else learning_rate # Get current or passed LR
            if optimizer is None and learning_rate is not None: # If optimizer not passed, create it
                 optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate)
            elif optimizer and learning_rate is not None and current_lr != learning_rate : # If LR changed in config
            ...
            start_epoch = checkpoint.get('epoch', latest_epoch) # epoch to resume FROM (so next epoch is start_epoch)
