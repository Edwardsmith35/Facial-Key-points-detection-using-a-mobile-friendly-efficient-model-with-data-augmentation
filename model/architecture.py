...
def build_facial_keypoints_model(pretrained: bool = True):
    ...
    """
    actual_weights = None # Default to no weights
    if pretrained:
        if PRETRAINED_WEIGHTS == 'MobileNet_V3_Small_Weights.IMAGENET1K_V1':
            ...
            # Handle unknown weight string or raise an error
            print(f"Warning: Unknown PRETRAINED_WEIGHTS string in config: '{PRETRAINED_WEIGHTS}'. Using no weights.")
            
    # Pass the actual weights object (or None) to the model constructor
...
def get_optimizer(model: nn.Module, learning_rate: float):
    ...
    # we only pass the model parameters that needs to be trained:
    return torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate)